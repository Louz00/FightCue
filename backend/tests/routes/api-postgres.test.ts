...
test("home endpoint deduplicates overlapping boxing cards and prioritizes richer official sources", async () => {
  const { app, close } = await createPostgresTestApp({
    configureRuntimeService(runtimeService) {
      runtimeService.getCachedTopRankPreview = async () =>
        buildBoxingPreview("top_rank", [
          createBoxingEvent({
      ...
      runtimeService.getCachedPbcPreview = async () =>
        buildBoxingPreview("pbc", [
          createBoxingEvent({
      ...
      runtimeService.getCachedGoldenBoyPreview = async () =>
        buildBoxingPreview("golden_boy", [
          createBoxingEvent({
      ...
      runtimeService.getCachedEspnBoxingPreview = async () =>
        buildBoxingPreview("espn_boxing", [
          createBoxingEvent({
...
test("home endpoint reuses a short-lived runtime snapshot for repeated identical requests", async () => {
  ...
  const { app, close } = await createPostgresTestApp({
    configureRuntimeService(runtimeService) {
      runtimeService.getCachedUfcPreview = async () => {
        ufcCalls += 1;
        return createEmptyPreview("ufc");
      };
      runtimeService.getCachedTopRankPreview = async () => {
        topRankCalls += 1;
        return createEmptyPreview("top_rank");
      };
    },
  });
...
function createOfflineRuntimeService(
  ...
  runtimeService.getCachedUfcPreview = async () => createEmptyPreview("ufc");
  runtimeService.getCachedGloryPreview = async () => createEmptyPreview("glory");
  runtimeService.getCachedMatchroomPreview = async () =>
    createEmptyPreview("matchroom");
  runtimeService.getCachedQueensberryPreview = async () =>
    createEmptyPreview("queensberry");
  runtimeService.getCachedTopRankPreview = async () =>
    createEmptyPreview("top_rank");
  runtimeService.getCachedPbcPreview = async () => createEmptyPreview("pbc");
  runtimeService.getCachedGoldenBoyPreview = async () =>
    createEmptyPreview("golden_boy");
  runtimeService.getCachedBoxxerPreview = async () =>
    createEmptyPreview("boxxer");
  runtimeService.getCachedEspnBoxingPreview = async () =>
    createEmptyPreview("espn_boxing");
